#!/usr/bin/env python3
"""Run Unified Search regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="maccms-backup-") as temporary:
        ...
        env_file.write_text("\n".join([
            ...
            "SEARCH_UNIFIED_REPRO=" + os.getenv("SEARCH_UNIFIED_REPRO", "0"),
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/framework_audit_unified_search.php",
