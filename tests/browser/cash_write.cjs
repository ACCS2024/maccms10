    ...
    try{
        ...
        server=http.createServer(async(req,res)=>{
            ...
            if(url.pathname==='/fixture/page'){
                ...
                if(!runs.has(id))runs.set(id,{mode:url.searchParams.get('mode'),requests:[],posts:0,tokens:0});
                res.setHeader('Set-Cookie','cash_run='+id+'; HttpOnly; SameSite=Lax; Path=/');
                res.setHeader('Content-Type','text/html; charset=utf-8');
                const client=url.searchParams.get('client');
                const markup='<div id="mac-user-cash-root" data-api-base="/fixture/api.php" data-write-token-url="/fixture/index.php/user/write_token">'+
            ...
            else check(form.get('cash_money')==='0.29'&&form.get('cash_bank_no')==='Ordinary+%20','Cash form must preserve money and literal account characters');
            ...
            setTimeout(()=>res.end(JSON.stringify({code:1,msg:'Saved'})),25);
        });
        ...
        async function open(client,mode){const id=String(++sequence);await page.goto(origin+'/fixture/page?run='+id+'&client='+client+'&mode='+mode);await page.waitForFunction(()=>window.ready);return runs.get(id);}
