#!/usr/bin/env python3
"""Run safe static video redirect regressions against disposable MySQL over a Unix socket."""
import os
            ...
            for image in images:
                for prefix in ["/", "/fixture/sub/"]:
                    subprocess.run([
                        ...
                        "-e", "STATIC_VOD_PREFIX=" + prefix,
                        "-v", str(root) + ":/app:ro", "-v", str(sockets) + ":/audit",
                        "-w", "/app", "--entrypoint", "php", image,
                        "/app/tests/framework_audit_static_vod.php",
                    ], check=True, timeout=120)
                    subprocess.run(["node", str(root / "tests/static_vod_redirects.js"), str(sockets / "static-vod-redirects.json")], check=True, timeout=15)
        finally:
            if started:
