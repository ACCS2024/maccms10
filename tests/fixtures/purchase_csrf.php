...
if(!defined('PURCHASE_CSRF_EXISTING_DB')&&(!$purchaseHttp||!is_file($purchaseTemp.'/schema.ready'))){
    ...
    foreach(array_merge(['user','group','plog','ulog','vod','art'],getenv('MANGA_PURCHASE_AUDIT')==='1'?['manga']:[],defined('CASH_WRITE_AUDIT')?['cash']:[])as $table){
...
function purchaseCsrfSeed():void{
    purchaseCsrfConfig();
    foreach(array_merge(['ulog','plog','user','group','vod','art'],getenv('MANGA_PURCHASE_AUDIT')==='1'?['manga']:[],defined('CASH_WRITE_AUDIT')?['cash']:[])as $table)Db::execute('DELETE FROM audit_'.$table);
    $groups=[];
...
function purchaseCsrfRoute(string $target,array $body=[],array $query=[],array $cookies=[],array $headers=[],string $method='POST',array $server=[]):array{
    global $app;
    $api=str_starts_with($target,'payment/')||str_starts_with($target,'cash/');$script=$api?'api.php':'index.php';
