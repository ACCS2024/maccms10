<?php
/** Loopback-only fixture for All's framework cache-hit response transport. No database/site bootstrap. */
...
$app->middleware->import([app\middleware\SecurityHeaders::class, think\middleware\SessionInit::class]);
$response=$app->middleware->pipeline()->send($r)->then(static function(){
    ...
    throw new RuntimeException('Expected cache-hit response');
});
$response->send();
