...
const source=fs.readFileSync(path.join(__dirname,'../static/js/manga-access.js'),'utf8');
let checks=0;
...
async function exercise(page,kind,responses,options={}){
  const markup=new RegExp('<form ([^>]*data-manga-'+kind+'[^>]*)>').exec(page.html);
  check(!!markup,'The requested interaction uses an actual rendered form');
  const attributes=attrs(markup[1]),base=attrs(/<div ([^>]*data-manga-base[^>]*)>/.exec(page.html)[1]);
  ...
  const root={getAttribute(){return options.base??base['data-manga-base'];},querySelector(name){if(name==='[data-manga-result]')return result;if(name==='[data-manga-'+kind+']')return form;return null;}};
  const location={href:'https://fixture.invalid/index.php/manga/play?id=1&sid=1&nid=1',origin:'https://fixture.invalid',reload(){reloads++;}};
...
(async()=>{
  const password=pages.find(page=>page.state==='password'),purchase=pages.find(page=>page.state==='purchase'),whole=pages.find(page=>page.state==='whole');
  const prefix=attrs(/<div ([^>]*data-manga-base[^>]*)>/.exec(password.html)[1])['data-manga-base'];
  ...
  const body=new URLSearchParams(pwd.request.body);check(body.get('pwd')==='a +&=0' && body.get('mid')==='12' && body.get('type')==='1' && body.get('id')==='1','Password bytes and the fixed article scope survive form encoding');
  ...
  for(const page of [purchase,whole]){
    ...
    check(data.get('csrf_token')==='fixture+&token' && data.get('mid')==='12' && data.get('type')==='1' && data.get('nid')===result.attributes['data-nid'] && data.get('sid')===result.attributes['data-sid'],'Only the rendered article coordinates and freshly fetched CSRF token are submitted');
    check(!data.has('points') && !data.has('user_id'),'The client cannot select the server price or charged identity');
    check(data.get('sid')==='1' && data.get('nid')==='1','Whole/current purchase sends an actual nonempty Manga chapter');
  }
  ...
  const long=pages.find(page=>page.state==='empty');check(!long.html.includes('data-manga-purchase') && !long.html.includes('SECRET-OTHER'),'Page coordinates outside the current voucher schema expose neither a purchase control nor protected text');
  console.log(`manga_resource_frontend: ${checks} checks passed on actual rendered HTML and shipped JavaScript`);
