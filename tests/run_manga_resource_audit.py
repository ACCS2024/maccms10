#!/usr/bin/env python3
"""Run real MySQL Manga resource authorization regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="maccms-backup-") as temporary:
        ...
        try:
            ...
            for image in images:
                for prefix in ["/", "/fixture/sub/"]:
                    subprocess.run([
                        "docker", "run", "--rm", "--network", "none", "--env-file", str(env_file),
                        "-e", "MANGA_AUDIT_PREFIX=" + prefix,
                        "-v", str(root) + ":/app:ro", "-v", str(sockets) + ":/audit",
                        "-w", "/app", "--entrypoint", "php", image,
                        "-d", "memory_limit=128M", "/app/tests/framework_audit_manga_access.php",
                    ], check=True, timeout=180)
                    subprocess.run(["node", str(root / "tests/manga_resource_frontend.js"), str(sockets / "manga-access-pages.json")], check=True, timeout=15)
        finally:
            if started:
