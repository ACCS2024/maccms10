#!/usr/bin/env python3
"""Run Public media visibility regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="maccms-backup-") as temporary:
        ...
        env_file.write_text("\n".join([
            ...
            "PUBLIC_CONTENT_POLICY_ONLY=" + os.getenv("PUBLIC_CONTENT_POLICY_ONLY", "0"),
            "PUBLIC_CONTENT_REPRO=" + os.getenv("PUBLIC_CONTENT_REPRO", "0"),
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/framework_audit_public_content_query.php",
