#!/usr/bin/env python3
"""Run Search suggestion regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="maccms-backup-") as temporary:
        ...
        env_file.write_text("\n".join([
            ...
            "SEARCH_SUGGEST_REPRO=" + os.getenv("SEARCH_SUGGEST_REPRO", "0"),
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/framework_audit_search_suggest.php",
