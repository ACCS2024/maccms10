#!/usr/bin/env python3
"""Run public content DTO regressions against disposable MySQL over a Unix socket."""
import os
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/framework_audit_api_content_view.php",
                ], check=True, timeout=120)
                subprocess.run(["node", str(root / "tests/public_content_default_renderers.js"),
                                str(sockets / "public-content-dtos.json")], check=True, timeout=20)
        finally:
            if started:
