#!/usr/bin/env python3
"""Run Receive/Collect regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    name = "maccms-audit-receive-" + token
    password = secrets.token_hex(24)
    database = "maccms_audit_receive_" + token
    with tempfile.TemporaryDirectory(prefix="maccms-receive-") as temporary:
        ...
        env_file.write_text("\n".join([
            "RECEIVE_COLLECT_AUDIT_MYSQL_SOCKET=/audit/mysql.sock",
            "RECEIVE_COLLECT_AUDIT_DATABASE=" + database,
            "RECEIVE_COLLECT_AUDIT_PASSWORD=" + password,
        ]) + "\n")
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/framework_audit_receive_collect.php",
