#!/usr/bin/env python3
"""Run Collection controller regressions against disposable MySQL over a Unix socket."""
import os
...
def main():
    ...
    name = "maccms-audit-collection-" + token
    password = secrets.token_hex(24)
    database = "maccms_audit_collection_" + token
    with tempfile.TemporaryDirectory(prefix="maccms-collection-") as temporary:
        ...
        env_file.write_text("\n".join([
            "COLLECTION_AUDIT_MYSQL_SOCKET=/audit/mysql.sock",
            "COLLECTION_AUDIT_DATABASE=" + database,
            "COLLECTION_AUDIT_PASSWORD=" + password,
        ]) + "\n")
        ...
        try:
            ...
            for image in images:
                subprocess.run([
                    ...
                    "/app/tests/extensions_audit_collection_controller.php",
