#!/usr/bin/env python3
"""Run article resource access regressions against disposable MySQL over a Unix socket."""
import os
            ...
            for image in images:
                for prefix in ["/", "/fixture/sub/"]:
                    subprocess.run([
                        ...
                        "-e", "ART_AUDIT_PREFIX=" + prefix,
                        "-v", str(root) + ":/app:ro", "-v", str(sockets) + ":/audit",
                        "-w", "/app", "--entrypoint", "php", image,
                        "/app/tests/framework_audit_art_access.php",
                    ], check=True, timeout=120)
                    subprocess.run(["node", str(root / "tests/art_resource_frontend.js"), str(sockets / "art-access-pages.json")], check=True, timeout=15)
        finally:
            if started:
