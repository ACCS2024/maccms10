...
- 2026-08-22 14:43 和 15:00 的自动采集验证均成功；这些是迁移期历史记录，不代表当前仍有自动任务。目标当时多出的 36 条来自该验证，后续改为手动同步。
...
6. 迁移期 `nei_internal_vod` 曾写入目标数据，虽然任务现已删除，仍禁止用旧库全量覆盖任何目标业务表。手动同步或旧机出现新写入时，只按业务键/更新时间生成并复核增量 SQL，再按字段交集合并；继续排除 `mac_admin` 和目标新增的 TP8 功能表。
