...
## DNS 切换（2026-09-09 完成）

### 🔴 切换前才发现的架构真相：不是一台源站，是 6 台
读 CF 解析记录才知道公网站点跑在一个集群上，而**我拿到的 23.225.146.130 只承载 2 个子域名**：

| 源站 IP | 承载 | 首页影片数 |
|---|---|---|
| 23.225.148.130 | json/json1-5.xingba222.com、xingba222.com、json1-5.xgbbk8.com | 112,078 |
| 23.225.148.131 | sex8zy1/2/3/4/6 | 112,078 |
| 23.225.148.132 | sex8zy7/8/9 | 112,078 |
| 23.225.148.133 | xingbazy.com | 112,078 |
| 23.225.148.134 | **sex8zy.com(主域)**、sex8zy5、xingba111.com | 112,078 |
| **23.225.146.130** | **仅** api.xgbbk8.com、json.xgbbk8.com（迁移来源） | 93,510（被删后） |

结论：**被删库的只有采集接口机，5 台前端机的库完好**（112,078 从没掉过）；两边是**各自独立的库**。
用 `/index.php/vod/detail/id/N.html`（前端机的 URL 方案）比对确认是**同源内容**：
前端机 max vod_id=142,249，新机 max=143,118 —— **采集接口机领先约 500 部，所以恢复基准正确、新机数据最全**。

> ⚠️ 教训：**切 DNS 前必须先读一遍真实解析记录**。只看"用户给的那台机器"会误判架构，
> 本次差点把「6 台合并成 1 台」当成「1 台换 1 台」。

### 前置准备
- **31 个域名**全部绑到新机（vhost `server_name` + 面板 domain 表一致，用 API `AddDomain` 而非手改 vhost，
  否则面板重写配置会丢）。
- **HTTPS**：主站域名全部 CF 橙云代理，且源站 443 有真证书 → CF 回源走 HTTPS，
  **新机不配 443 就会全站白页**。从源站迁了那张 Let's Encrypt 证书（29 SAN，到期 2026-10-10），
  实测 7 个域名 `ssl_verify=0`。
- **无需末次同步**：老机 `vod_id > 127708` 的新增为 **0**，新机数据严格优于老机。

### 执行
1. 全量备份 16 个 zone / 46 条记录 → 新机 `/root/dns-backup/cf-dns-before-cutover-20260909.json`
2. **灰度**：先切 `sex8zy9.com` + www，经 CF 验证 8 个端点全 200、内容是新机的（143118），再继续
3. **批量**：29 条 A 记录 `→ 85.149.233.2`（保持 proxied），**成功 29 / 失败 0**
4. 验证：17 个域名多次采样，16 个 3/3 收敛；`cf-cache-status: DYNAMIC`（未走缓存），
   偶发旧值是 **CF 边缘传播延迟**，直连新机三个域名恒为 112,463 —— 源站侧无问题

### 保持不动（需另行决策）
- `json1-5.xingba222.com` / `json1-5.xgbbk8.com` 共 **10 条**仍指向 23.225.148.130 ——
  **新机没绑这些子域名**，切了会落到默认站。要迁需先在新机 `AddDomain`。
- `xbww888.com` / `xbxbw888.com` 是 **CNAME → `66f13f98.pigeonnscf.com`（第三方 CDN，未走 CF 代理）**，
  播放器站流量大，动它等于摘掉一层 CDN，未动。

### 回滚
CF 记录改回快照里的原 IP 即可（每条记录的 zone/id/原值都在备份 JSON 里）。
**6 台老机全部保持在线未停**，随时可切回。

### CF 令牌
切 DNS 需要 **区域(Zone) → DNS → 编辑** 权限 + 区域资源选「所有区域」。
账户级的 `Account DNS Settings` / `DNS Firewall` / `DNS View` 都**不是**改解析记录的权限。
