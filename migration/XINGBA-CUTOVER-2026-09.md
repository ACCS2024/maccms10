...
### 教训：冒烟测试必须爬页面真实生成的链接，而不是自己拼 URL
改用爬虫从渲染结果里抽 `href` 逐个测，3 层深度覆盖
首页 / 分页 / 分类页 / 分类分页 / 文章分类 / 详情页 / 播放页 / 帮助页 / 会员页 / 静态资源，
**34 个真实链接全部 2xx**；并且要**核对页面正文非空**（影片链接数、可见文本长度），
不能只看状态码。首页修复后 56,676 B → 116,838 B（源站 118,632 B）。
