...
卡密 use_card 的实际 Request/Json PHPDoc 也已修正；定向 Payment level 5 从 44 条降为 37 条，7 条该方法文档候选关闭，包含 cc2d60bf 新增的两条返回分支提示。没有改变运行时类型和方法体。XLSX 回调状态及其它 API 文档继续保留复核，不能将定向数值作为新一轮全量计数。

## 4a7f90c7 重扫

见[刷新基线](full-repair-baseline-20260911.md)：level 1 为 215、level 5 为 978，均无全局分析错误，PHPCompatibility 0 错误、8 警告。旧可执行更新 SQL 和来源治理批次已提交，不能继续把那些已删除入口算作当前候选；也不能把减少的诊断数直接算作已修漏洞数。动态门面和运行期常量仍未加入忽略规则。后续封面修复有独立提交与实际事务证据；全量静态数值仍明确对应 4a7f90c7。
