...
日期：2026-09-11，最新完整默认/编译/静态基线见 [`c5ae4121` 检查点](full-repair-checkpoint-c5ae4121.md)；完整 MySQL 分组基线见 [`4a7f90c7` 刷新记录](full-repair-baseline-20260911.md)，后续关键事务均另跑真实 MySQL，见行内记录。此表区分已提交修复、实际复现和待核实范围，避免把一个入口通过回归当成整个系统已解决。每项修复仍独立冻结、验证和提交。
...
| TP8 内容导入（I1） | [I1a 行准备](import-row-preparation.md)和[源行坐标](import-source-rows.md)已提交；[I1b 接收及结果](content-import-ingress.md)恢复真实 TP8 单文件上传、三动作返回、部分成功及保存结果不明时停止；双版 multipart 36 项、Art/Manga 每库各 81 项、Vod MySQL 85 项 | [I1c 列映射](import-column-mapping.md)已完成整文件预检及行列定位；双版解析 81 项，Art/Manga 每库 102、Vod MySQL 106 项。[I1d 文本预检](import-text-encoding.md)已补齐，双版解析 78 项，Art/Manga 每库 114、Vod MySQL 118 项；模型零影响行、落库截断、行原子性和持久核对尚未闭环。Domain/Cj/Vodplayer 的旧上传对象及配置数据合同另审 |
...
| 配置解析资源上限 | `9b0006ad` 提交独立 DataConfig 解析器，在原生分词前计预算；双版各 101 项、128M 隔离容量检查 | `e2dfef32` 已提交站点/插件加载链和原子写入；双版 DataConfig 174 项，配置保留 14 项。新入口及其它非配置执行点继续复审 |
| 前端与配置来源治理 | `588b871a` 至 `4a7f90c7` 已拆为 14 批并验证、提交；清理结束时 Git 工作区与暂存区均干净，见[记录](workspace-cleanup-20260911.md) | 真实发布、生产恢复和线上 CI 未执行；来源检查、浏览器和服务器合同须随后续变更保留回归 |
