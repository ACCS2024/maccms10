...
## 阶段状态更新：视频链（2026-09-10）

A1 已提交 `8aaea1d`，A2a 已提交 `9fee023`。视频子批已提交 `06e2bd2`，响应合同、真实验证及限制见 [vod-resource-access.md](vod-resource-access.md)；本文件其余“待修”表格保留发现时证据。视频 API/前台当前资源、下一集、完整下载目录、三个独立密码 scope、旧版 Ajax 视频列表和现有目录模板已纳入该子批。购买写入已有根代理的独立修复，仍需与 `ContentResource::vodContext` 对接真实坐标/服务器价格，不能以读方完成代替购买集成。

**接下来的提交顺序：安全静态播放/下载页面 → Art 完整正文/密码链 → Manga 完整章节/密码链。** 静态 Make view 2/3/4 原先跳过权限并写入原始资源，视频提交先在写文件和更新进度前明确失败。后续独立静态组现以本地外部 JS、公开坐标和固定数字 ID 动态授权入口恢复既有静态链接，保留静态详情公开目录；具体功能、文件失败处理、真实产物验证及限制见 [static-vod-redirects.md](static-vod-redirects.md)，等待根代理独立提交。已生成的旧 HTML/CDN 副本需部署处置，本轮不会操作用户站点文件。

新增旁路证据：`index/Ajax::data(mid=1)` 原先只去媒体原址而保留三种原始密码；`All::label_vod_role` 原先给模板整条记录。两个出口已随视频子批收口。相关 Art 列表/正文、Manga 验证与章节输出仍是后续必做，采集导出配置鉴权、元数据状态策略及 `vod_lock/copyright_status` 语义不作为视频积分/密码子批已完成事项。
