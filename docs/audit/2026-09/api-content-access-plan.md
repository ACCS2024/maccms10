...
A1 已提交 `8aaea1d`，A2a 已提交 `9fee023`。当前视频子批的响应合同、真实验证及限制见 [vod-resource-access.md](vod-resource-access.md)，等待根代理独立审阅提交；本文件其余“待修”表格保留发现时证据。视频 API/前台当前资源、下一集、完整下载目录、三个独立密码 scope、旧版 Ajax 视频列表和现有目录模板已纳入该子批。购买写入已有根代理的独立修复，仍需与 `ContentResource::vodContext` 对接真实坐标/服务器价格，不能以读方完成代替购买集成。

**接下来的提交顺序：安全静态播放/下载页面 → Art 完整正文/密码链 → Manga 完整章节/密码链。** 静态 Make view 2/3/4 原先跳过权限并写入原始资源，本批在写文件和更新进度前明确失败；这是短期阻断，下一子批必须用动态授权入口的安全跳转/外壳恢复既有静态链接，保留静态详情的公开目录。已生成的旧 HTML/CDN 副本需部署处置，本轮不会操作用户站点文件。
