...
**后台还原冲突守卫**：insert-fresh 会在回收站留下旧记录。若后台用户此时去回收站【还原】旧记录，
会与已重推的活跃版撞成两条同名活跃行。为此 `admin/Vod::restore()` 加了冲突检测：还原时若已存在
同名活跃版本，弹窗「已存在另一版本，是否覆盖？」——**是**=把现有活跃版移回回收站、让被还原版上位
（可回滚、不物理删除）；**否**=取消还原。前后端协议：后端返回 `{code:2, confirm:'…'}`，前端
`admin_common.js` 据此二次确认并带 `overwrite=1` 重发（opt-in，不影响其它后台动作）。
