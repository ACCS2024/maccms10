...
- **`/think` 静态源码可见(⚪ 已实测 200)**:`think` 无扩展名,Apache 默认按静态文件下发其 PHP **源码**(开源、不含任何密钥);**执行**已被 `PHP_SAPI!=='cli'` 守卫阻断,故无 RCE/触达风险。若要连源码也藏,见 §12.5 的 web 服务器 deny。

### 12.4b Web 冒烟实测(Apache + AllowOverride All,2026-06)

| 路径 | HTTP | 结论 |
|---|---|---|
| `/application/database.php` | **403** | 既有 deny ✓ |
| `/runtime/backup/secret.sql` | **403** | 新增 runtime/.htaccess ✓(修复备份外泄) |
| `/bin/maccms`、`/bin/_config.php` | **403** | 新增 bin/.htaccess ✓ |
| `/think` | 200(源码) | 仅源码可见,执行被 SAPI 守卫挡;见上 ⚪ |
| `/`(首页) | 302 | 站点正常(跳安装) |

CLI 冒烟:install→info(连接✓/53表)→db:export(落 `application/data/backup/`)→cache:flush→admin:reset-password→db:search-replace 均通过,SAPI 守卫不影响 CLI。

### 12.5 部署侧建议(纵深防御)

- Apache:已随仓库提供 `application/.htaccess`、`bin/.htaccess`、`runtime/.htaccess`。如需连 `/think` 源码也拒绝,在站点根配置/vhost(不随仓库下发以免冲突重写规则)加:
  ```apache
  <Files "think"> Require all denied </Files>
  ```
- **Nginx**(不读 .htaccess,**必须**显式配):在 server 段补:
  ```nginx
