...
var MAC = {
        ...
        'Init': function ($obj, $mid, $jumpurl) {
            try {
                $($obj).autocomplete(maccms.path + '/index.php/ajax/suggest?mid=' + $mid, {
                    ...
                    width: 430, scrollHeight: 439, minChars: 2, delay: 450, matchSubset: 0,
                    cacheLength: 80, multiple: false, matchContains: true, autoFill: false,
                    dataType: "json",
                    parse: function (r) {
                        if (r.code == 1) {
                            var parsed = [];
    ...
    'Pwd': {
        'Check': function (o) {
            var $that = $(o);
            if ($that.attr("data-id")) {
                MAC.Ajax(maccms.path + '/index.php/ajax/pwd.html?id=' + $that.attr("data-id") + '&mid=' + $that.attr("data-mid") + '&type=' + $that.attr("data-type") + '&pwd=' + encodeURIComponent($that.parents('form').find('input[name="pwd"]').val()), 'get', 'json', '', function (r) {
                    $that.addClass('disabled');
                    MAC.Pop.Msg(300, 50, r.msg, 2000);
                    if (r.code == 1) {
                        location.reload();
