...
class Art extends Base {
    ...
    public function listData($where,$order,$page=1,$limit=20,$start=0,$field='*',$addition=1,$totalshow=1)
    {
        ...
        $list = Db::name('Art')->field($field)->where($where)->where($where2)->order($order)->limit($offset, $limit)->select()->toArray();
        //dump($where);die;
    ...
    public function listCacheData($lp)
    {
        ...
        if($GLOBALS['config']['app']['cache_core']==0 || empty($res)) {
            if ($meili !== null) {
                ...
                $res = $this->listData($where, $order, 1, $num, 0, '*', 1, 0);
                $res['page'] = $page;
                if ($totalshow == 1) {
    ...
    public function fieldData($where,$update)
    {
        ...
        $list = $this->field('art_id,art_name,art_en')->where($where)->select()->toArray();
        foreach($list as $k=>$v){
