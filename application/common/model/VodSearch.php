...
class VodSearch extends Base {
    ...
    public function checkAndUpdateTopResults($vod, $force = false)
    {
        ...
        if (is_null($list)) {
            ...
            if (empty($list)) {
                $list = $this->field("search_key, search_word, search_field")->order("search_hit_count DESC, search_last_hit_time DESC")->limit(0, (int)$this->updateTopCount)->select()->toArray();
