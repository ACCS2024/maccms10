...
/** Resolves manga chapter, current price and the established read policy inside one financial transaction. */
final class MangaPurchase
{
    ...
    public static function buy(array $identity, array $parameters, callable $access): array
    {
        ...
        if ($selection===null || $selection['mid']!==12 || $selection['sid']<1 || $selection['nid']<1) {
            return ['code'=>2001,'msg'=>lang('param_err')];
        }
        foreach (['manga_points_type'=>0,'status'=>1] as $field=>$default) {
            if (!in_array(PointsBalance::amount($GLOBALS['config']['user'][$field]??$default,true),[0,1],true)) {
                return ['code'=>2001,'msg'=>lang('param_err')];
            }
        }
        return ContentPurchase::buyManga($identity['user_id']??null,static function(array $user) use($identity,$selection,$access): array {
            ...
            $row=Db::name('Manga')->master()->where('manga_id',$selection['id'])->lock(true)->find();
            $whole=(string)($GLOBALS['config']['user']['manga_points_type']??'0')==='1';
            $priceField=$whole?'manga_points':'manga_points_detail';
            if (!$row || PointsBalance::amount($row['manga_status']??null,true)!==1
                || PointsBalance::amount($row['manga_recycle_time']??null,true)!==0
                || PointsBalance::amount($row[$priceField]??null,true)===null
                || (!$whole && (int)$row[$priceField]===0 && PointsBalance::amount($row['manga_points']??null,true)===null)) {
                return ['code'=>1002,'msg'=>lang('obtain_err')];
            }
            // A purchase must refer to an actual nonempty chapter; Manga never clamps missing coordinates.
            $context=ContentResource::mangaContext($row,['sid'=>$selection['sid'],'nid'=>$selection['nid']]);
            if ($context['code']!==1 || $context['points']>65535 || ($context['images']??[])===[]) {
                return ['code'=>1002,'msg'=>lang('obtain_err')];
            }
            ...
            $enabled=[];$currentPermission=false;
            foreach ($groups as $group) {
                $gid=$group['group_id'];$enabled[]=$gid;
                $current=self::groupPermission($group,(int)$row['type_id']);
                if ($current===null) { return ['code'=>3001,'msg'=>lang('controller/no_popedom')]; }
                $currentPermission=$currentPermission || $current;
            }
            $record=['user_id'=>(int)$user['user_id'],'ulog_mid'=>12,'ulog_type'=>1,'ulog_rid'=>$context['id'],
            ...
            try {
                $GLOBALS['user']=$user;
                $permission=$access($row,['id'=>$context['id'],'sid'=>$context['sid'],'nid'=>$context['nid']]);
            } finally {
            ...
            if (($permission['can_access']??false)===true) {
                // The actual read policy must agree with the locked quote before returning a free entitlement.
            ...
            // Unlike video, Manga explicitly permits a member without category read permission to buy this chapter.
