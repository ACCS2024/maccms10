...
final class ContentPurchase
{
    ...
    private static function execute($userId, array $pricedRecord, ?callable $quote = null, ?int $resourceMid = null): array
    {
        ...
        $ownerScope=null;
        if (($blocked = PurchaseTransaction::blockedResult()) !== null) { return $blocked; }
        try {
            ...
            if ($quote !== null) {
                ...
                $ownerScope = new PurchaseTransaction($userId, $record['ulog_mid'], true);
                $ownerScope->begin();
            }
            ...
            return $ownerScope->commit(['code'=>1,'msg'=>lang('save_ok')]);
